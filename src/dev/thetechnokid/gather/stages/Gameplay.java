...
public class Gameplay extends Stage {
	...
	private void createEntities() {
		for (int y = 0; y < Game.getCurrentGame().getScreen().height / Tile.DRAW_SIZE; y++) {
			for (int x = 0; x < Game.getCurrentGame().getScreen().width / Tile.DRAW_SIZE; x++) {
				if (Math.random() < .3 && Math.random() < .3) {
					Ore c = new Ore();
					c.x = x * Tile.DRAW_SIZE;
					c.y = y * Tile.DRAW_SIZE;
					Game.getCurrentGame().getScreen().getController().addEntity(c);
				}
			}
		}

		if (!Game.getCurrentGame().getScreen().getController().getEntities()
				.contains(Game.getCurrentGame().getScreen().getController().getUser())) {
			Game.getCurrentGame().getScreen().getController()
					.addEntity(Game.getCurrentGame().getScreen().getController().getUser());
		}
	}
	...
	public void tick() {
		...
		boolean done = false;
		for (Entity entity : Game.getCurrentGame().getScreen().getController().getEntities()) {
		...
		if (done) {
			for (Entity e : Game.getCurrentGame().getScreen().getController().getEntities()) {
				if (!(e instanceof Player))
					Game.getCurrentGame().getScreen().getController().getEntities().remove(e);
			}
			createEntities();
		}

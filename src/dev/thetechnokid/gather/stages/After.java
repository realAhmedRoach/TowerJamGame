...
public class After extends Stage {
	...
	public void render(Graphics g, Screen screen) {
		Text.render("You have " + (lost ? "lost" : "won"), g,
				5 * Tile.DRAW_SIZE, 3 * Tile.DRAW_SIZE);
		if (lost) {
			int[] lose = lose();
			Text.render("You lost " + lose[0] + "C and " + lose[1] + "I", g,
	...
	private int[] lose() {
		int loseAmtC = (int) Math.random()
				* (Game.getCurrentGame().getLogicManager().getCoal() / 2);
		int loseAmtI = (int) Math.random()
				* (Game.getCurrentGame().getLogicManager().getIron() / 2);
	...
	private int[] win() {
		int winAmtC = (int) Math.random()
				* (Game.getCurrentGame().getLogicManager().getCoal() / 2);
		int winAmtI = (int) Math.random()
				* (Game.getCurrentGame().getLogicManager().getIron() / 2);

...
public class Store {
	...
	private static void checkArmor() {
		if (Game.getCurrentGame().getKeyboard().getKeys()[KeyEvent.VK_V]) {
			if (Game.getCurrentGame().getLogicManager().getArmorTier() >= 1)
				return;
			if (Game.getCurrentGame().getLogicManager()
					.buy(Store.PRICE_ARMORT1[0], Store.PRICE_ARMORT1[1]))
				Game.getCurrentGame().getLogicManager().upgradeArmor(1);
			...
			if (Game.getCurrentGame().getLogicManager()
				...
				Game.getCurrentGame().getLogicManager().upgradeArmor(3);
		}
	}

	private static void checkSwords() {
		if (Game.getCurrentGame().getKeyboard().getKeys()[KeyEvent.VK_G]) {
			if (Game.getCurrentGame().getLogicManager().getSwordTier() >= 1)
				return;
			if (Game.getCurrentGame().getLogicManager()
					.buy(Store.PRICE_SWORDT1[0], Store.PRICE_SWORDT1[1]))
				Game.getCurrentGame().getLogicManager().upgradeSword(1);
		} else if (Game.getCurrentGame().getKeyboard()
				.getKeys()[KeyEvent.VK_H]) {
			if (Game.getCurrentGame().getLogicManager().getSwordTier() >= 2)
				return;
			if (Game.getCurrentGame().getLogicManager()
					.buy(Store.PRICE_ARMORT2[0], Store.PRICE_ARMORT2[1]))
				Game.getCurrentGame().getLogicManager().upgradeSword(2);
		} else if (Game.getCurrentGame().getKeyboard()
				.getKeys()[KeyEvent.VK_J]) {
			if (Game.getCurrentGame().getLogicManager().getSwordTier() >= 3)
				return;
			if (Game.getCurrentGame().getLogicManager()
					.buy(Store.PRICE_ARMORT3[0], Store.PRICE_ARMORT3[1]))
				Game.getCurrentGame().getLogicManager().upgradeSword(3);
		}
	}
}
